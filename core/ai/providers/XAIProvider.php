 ...
 */
class XAIProvider implements LLMProviderInterface
{
    ...
    public function send(string $model, string $systemPrompt, string $userPrompt, array $options = []): array
    {
        ...
        if (isset($response['error'])) {
            throw new \RuntimeException('xAI API error: ' . ($response['error']['message'] ?? json_encode($response['error'])));
        }
    ...
    private function request(array $body): array
    {
        ...
        if ($error) {
            throw new \RuntimeException('xAI request failed: ' . $error);
        }

        if ($httpCode >= 400) {
            $decoded = json_decode($result, true);
            $msg = $decoded['error']['message'] ?? "HTTP {$httpCode}";
            throw new \RuntimeException('xAI API error: ' . $msg);
        }
